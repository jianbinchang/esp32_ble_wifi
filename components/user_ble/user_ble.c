            ...
            if (adv_name != NULL) {
                if (strlen(remote_device_name) == adv_name_len && strncmp((char *)adv_name, remote_device_name, adv_name_len) == 0) {
                    ...
                    if (connect == false) {
                        ...
                        if(memcmp(scan_result->scan_rst.bda, save_bound_add, ESP_BD_ADDR_LEN) == 0)
                        {
                            connect = true;
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        a_property = ESP_GATT_CHAR_PROP_BIT_READ | ESP_GATT_CHAR_PROP_BIT_WRITE_NR | ESP_GATT_CHAR_PROP_BIT_WRITE| ESP_GATT_CHAR_PROP_BIT_NOTIFY; //ESP_GATT_CHAR_PROP_BIT_WRITE_NR
    ...
    case ESP_GATTS_CONNECT_EVT: {
        ...
        {
            ESP_LOGI(GATTS_TAG, "ESP_GATTS_CONNECT_EVT break carll test ...\n");
            break;
        }
